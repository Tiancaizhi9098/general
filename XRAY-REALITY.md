...
```bash
(command -v dnf >/dev/null 2>&1 && dnf install -y curl ca-certificates openssl jq iproute || yum install -y curl ca-certificates openssl jq iproute) && bash -c "$(curl -fsSL 'https://raw.githubusercontent.com/Tiancaizhi9098/general/main/xray-reality-installer.sh?v=20260728.3')"
```
...
NODE_NAME=香港落地 \
bash -c "$(curl -fsSL 'https://raw.githubusercontent.com/Tiancaizhi9098/general/main/xray-reality-installer.sh?v=20260728.3')"
```
...
```bash
curl -fsSL 'https://raw.githubusercontent.com/Tiancaizhi9098/general/main/xray-reality-installer.sh?v=20260728.3' -o install.sh
less install.sh
bash install.sh
```
